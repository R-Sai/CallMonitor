...
class CallLogRepositoryImpl @Inject constructor(
    ...
    override suspend fun putCallLog(callLog: CallLogDomainModel): Flow<Long> {
        return flowOf(dao.insert(mapper.map(callLog)))
    }

    override suspend fun getCallLog(): Flow<List<CallLogDomainModel>> {
        return dao.getCallLogs().map { entities ->
            entities.map { mapper.map(it) }
        }
    }

    override suspend fun updateCallLogs(callLogs: List<CallLogDomainModel>) {
